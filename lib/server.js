...
server.on("request", function (request, response) {
    ...
    if (targetURL === "/") {
        ...
        connection.query(getDataQuery, function (err, rows) {
            if (err) {
                console.log("err: " + err);
            }
            //ファイル読み込み
        ...
        request.on("end", function () {
            ...
            if (Object.keys(errors).length > 0) {
                ...
                connection.query(checkQuery, function (err, rows) {
                    ...
                    if (rows.length > 0) {
                        // アンカーをつけて、トップページに戻る。
                        response.writeHead(302, {"location": "/#" + rows[0]["id"]});
                        response.end();
                    } else { // 歌手名と曲名の組み合わせがDBにない時
                        //とりあえずDBに歌手名と曲名をインサートしてlyricとmp3pathはNULLのままほっておく
                        var insertSingerNameQuery =
                            "INSERT INTO singer(name) " +
                            "VALUES(\"" + inputs["singerName"] + "\");";
                        connection.query(insertSingerNameQuery, function () {
                            connection.query("SELECT LAST_INSERT_ID()", function (singerID) {
                                var insertTitleQuery =
                                    "INSERT INTO song(title) " +
                                    "VALUES(\"" + inputs["title"] + "\");";
                                connection.query(insertTitleQuery, function () {
                                    connection.query("SELECT LAST_INSERT_ID()", function (songID) {
                                        var insertSingerIdAndSongIdQuery =
                                            "INSERT INTO singer_song(singer_id, song_id) " +
                                            "VALUES(\"" + singerID + "\", \"" + songID + ");";
                                        connection.query(insertSingerIdAndSongIdQuery);
                                    });
                                });
                            });
                        });
