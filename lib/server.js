...
server.on("request", function (request, response) {
    ...
    if (targetURL === "/") {
        ...
        request.on("end", function () {
            // パースしてinputsを作る
            var hoge = dataFromInputForms.split("------WebKitFormBoundary");
            var inputs = {};
            inputs["title"] = hoge[1].split(/\r\n|\r|\n/)[3];
            inputs["singerName"] = hoge[2].split(/\r\n|\r|\n/)[3];
            inputs["mp3file"] = hoge[3].split(/\r\n|\r|\n/)[1].split(";")[2].split("=")[1].slice(1, -1);
            var fuga = hoge[3].split(/\r\n|\r|\n/);
            for (var i = 0; i < 4; i++) {
                fuga.shift()
            }
            inputs["mp3binary"] = fuga.join("");
            ...
            if (Object.keys(errors).length > 0) {
                ...
                connection.query(checkQuery, [inputs["singerName"], inputs["title"]], function (err, rows) {
                        ...
                        if (rows.length > 0) {
                            ...
                            // mp3の保存
                            fs.writeFile("/Users/Yudai/dirForLyrics/lib/uploaded/" + inputs["mp3file"], inputs["mp3binary"]);

                            /* ここをhttps://www.html5rocks.com/ja/tutorials/file/dndfiles/でいい感じにする
                            var form = new formidable.IncomingForm();
                            form.parse(request, function (err, fields, files) {
                                console.log("2222222222222222222222222222222222222222")
                                var filePath = files["file"]["path"]
                                var realName = files["file"]["name"]
                                var target = "/Users/Yudai/dirForLyrics/lib/uploaded/";
                                var source = fs.createReadStream(filePath);
                                var dest = fs.createWriteStream(target + realName);
                                source.pipe(dest);
                            ...
                            */
                            ...
                            //とりあえずDBに歌手名と曲名とmp3pathをインサートしてlyricはNULLのままほっておく
                                ...
                                connection.query("SELECT LAST_INSERT_ID()", function (err, rows) {
                                    ...
                                    connection.query("INSERT INTO song(title) VALUES(?);", [inputs["title"]], function () {
                                        connection.query("SELECT LAST_INSERT_ID()", function (err, rows) {
                                            ...
                                            connection.query("INSERT INTO singer_song(singer_id, song_id, mp3path) VALUES(?, ?, ?)", [singerID, songID, "uploaded/" + inputs["mp3file"]]);
                                        });
                                    });
                                });
                            });
