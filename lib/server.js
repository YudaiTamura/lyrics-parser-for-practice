            ...
            if (Object.keys(errors).length > 0) {
                ...
                connection.query(checkQuery, function (err, rows) {
                    ...
                    if (rows.length > 0) {
                        ...
                        //とりあえずDBに歌手名と曲名をインサートして歌詞はNULLのままほっておく
                        var insertSingerNameQuery =
                            "INSERT INTO singer (singer.name)" +
                            "VALUES (\"" + inputs["singerName"] + "\");";
                        connection.query(insertSingerNameQuery);
                        var insertTitleQuery =
                            "INSERT INTO song (song.title)" +
                            "VALUES (\"" + inputs["title"] + "\");";
                        connection.query(insertTitleQuery);
