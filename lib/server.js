            ...
            if (Object.keys(errors).length > 0) {
                ...
                connection.query(checkQuery, function (err, rows) {
                    ...
                    if (rows.length > 0) {
                        ...
                        //とりあえずDBに歌手名と曲名を登録して歌詞はNULLのままほっておく
                        var insertSingerNameAndTitleQuery =
                            "INSERT lyrics_database.song (singer.name, song.title)" +
                            "VALUES (\"" + inputs["singerName"] + "\", \"" + inputs["title"] + "\")" +
                            "INNER JOIN singer_song ON singer_song.song_id = song.id " +
                            "INNER JOIN composer ON composer.id = song.composer_id " +
                            "INNER JOIN singer ON singer.id = singer_song.singer_id;";
                        connection.query(insertSingerNameAndTitleQuery, function (error, array) {}
                    }
                });
            }
        });
